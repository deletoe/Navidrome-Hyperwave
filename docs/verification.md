...
## Playback gain-envelope acceptance

The 2026-07-15 audio transition pass layers a transient `0–1` envelope over the saved user volume without publishing per-tick React state. A user-initiated play starts the media element at zero effective volume and reaches the latest slider value through a 320ms smoothstep fade. Pause changes the UI state immediately, follows a 240ms smoothstep fade to silence, then invokes the native media pause exactly once and restores the silent element's target volume for the next start.

Fake-timer coverage verifies fade-in, fade-out, changing the user volume mid-fade, reversing a pending fade-out, cancelling an unresolved play through a second toggle, completing either envelope safely when the page becomes hidden, immediate pause while muted or at zero volume, cancelling the envelope during continuous track changes, preserving a newer fade-out when an older play promise settles, and clearing every timer on unmount. Generation and playback-intent guards prevent an old timeout or stale play promise from pausing a newer track. Natural next-track continuation stays full-volume; reset, media error and queue clearing remain immediate rather than delaying destructive cleanup for an audible effect.
